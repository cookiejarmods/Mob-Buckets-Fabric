...
public class MooshroomBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_mooshroom";

    public MooshroomBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the mooshroom
            MooshroomEntity mooshroom = spawn(EntityType.MOOSHROOM, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (mooshroom != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    mooshroom.readCustomDataFromTag(data);
