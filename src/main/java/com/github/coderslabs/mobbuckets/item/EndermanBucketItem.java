...
public class EndermanBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_enderman";

    public EndermanBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the enderman
            EndermanEntity enderman = spawn(EntityType.ENDERMAN, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (enderman != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    enderman.readCustomDataFromTag(data);
