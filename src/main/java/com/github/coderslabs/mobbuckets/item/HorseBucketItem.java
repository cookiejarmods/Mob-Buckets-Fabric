...
public class HorseBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_horse";

    public HorseBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the horse
            HorseEntity horse = spawn(EntityType.HORSE, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (horse != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    horse.readCustomDataFromTag(data);
