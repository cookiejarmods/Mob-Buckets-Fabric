...
public class PolarBearBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_polar_bear";

    public PolarBearBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the polarBear
            PolarBearEntity polarBear = spawn(EntityType.POLAR_BEAR, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (polarBear != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    polarBear.readCustomDataFromTag(data);
