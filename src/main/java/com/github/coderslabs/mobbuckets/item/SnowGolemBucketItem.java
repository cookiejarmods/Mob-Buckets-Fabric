...
public class SnowGolemBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_snow_golem";

    public SnowGolemBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the snowGolem
            SnowGolemEntity snowGolem = spawn(EntityType.SNOW_GOLEM, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (snowGolem != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    snowGolem.readCustomDataFromTag(data);
