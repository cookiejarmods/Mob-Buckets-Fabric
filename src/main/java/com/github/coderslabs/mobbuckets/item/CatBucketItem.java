...
public class CatBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_cat";

    public CatBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the cat
            CatEntity cat = spawn(EntityType.CAT, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (cat != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    cat.readCustomDataFromTag(data);
