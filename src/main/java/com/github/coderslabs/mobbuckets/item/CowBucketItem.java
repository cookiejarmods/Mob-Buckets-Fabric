...
public class CowBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_cow";

    public CowBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the cow
            CowEntity cow = spawn(EntityType.COW, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (cow != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    cow.readCustomDataFromTag(data);
