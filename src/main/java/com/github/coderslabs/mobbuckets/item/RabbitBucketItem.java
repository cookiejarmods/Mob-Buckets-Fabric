...
public class RabbitBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_rabbit";

    public RabbitBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the rabbit
            RabbitEntity rabbit = spawn(EntityType.RABBIT, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (rabbit != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    rabbit.readCustomDataFromTag(data);
