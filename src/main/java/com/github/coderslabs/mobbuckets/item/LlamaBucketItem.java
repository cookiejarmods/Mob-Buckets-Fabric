...
public class LlamaBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_llama";

    public LlamaBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the llama
            LlamaEntity llama = spawn(EntityType.LLAMA, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (llama != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    llama.readCustomDataFromTag(data);
