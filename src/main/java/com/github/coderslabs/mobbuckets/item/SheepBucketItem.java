...
public class SheepBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_sheep";

    public SheepBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the sheep
            SheepEntity sheep = spawn(EntityType.SHEEP, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (sheep != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    sheep.readCustomDataFromTag(data);
