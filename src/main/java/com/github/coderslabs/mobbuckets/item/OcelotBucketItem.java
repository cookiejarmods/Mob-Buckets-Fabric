...
public class OcelotBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_ocelot";

    public OcelotBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the ocelot
            OcelotEntity ocelot = spawn(EntityType.OCELOT, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (ocelot != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    ocelot.readCustomDataFromTag(data);
