...
public class WolfBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_wolf";

    public WolfBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the wolf
            WolfEntity wolf = spawn(EntityType.WOLF, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (wolf != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    wolf.readCustomDataFromTag(data);
