...
public class IronGolemBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_iron_golem";

    public IronGolemBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the ironGolem
            IronGolemEntity ironGolem = spawn(EntityType.IRON_GOLEM, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (ironGolem != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    ironGolem.readCustomDataFromTag(data);
