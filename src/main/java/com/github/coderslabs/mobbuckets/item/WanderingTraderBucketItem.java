...
public class WanderingTraderBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_wandering_trader";

    public WanderingTraderBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the trader
            WanderingTraderEntity trader = spawn(EntityType.WANDERING_TRADER, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (trader != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    trader.readCustomDataFromTag(data);
