...
public class SquidBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_squid";

    public SquidBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the squid
            SquidEntity squid = spawn(EntityType.SQUID, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (squid != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    squid.readCustomDataFromTag(data);
