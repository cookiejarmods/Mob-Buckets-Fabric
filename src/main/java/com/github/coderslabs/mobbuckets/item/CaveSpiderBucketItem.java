...
public class CaveSpiderBucketItem extends Item {
    ...
    public static final String STORED_MOB = "stored_cave_spider";
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the caveSpider
            CaveSpiderEntity caveSpider = spawn(EntityType.CAVE_SPIDER, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (caveSpider != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    caveSpider.readCustomDataFromTag(data);
