...
public class MuleBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_mule";

    public MuleBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the mule
            MuleEntity mule = spawn(EntityType.MULE, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (mule != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    mule.readCustomDataFromTag(data);
