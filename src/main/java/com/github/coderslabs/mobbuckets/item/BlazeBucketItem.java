...
public class BlazeBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_blaze";

    public BlazeBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the sheep
            BlazeEntity blaze = spawn(EntityType.BLAZE, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (blaze != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    blaze.readCustomDataFromTag(data);
