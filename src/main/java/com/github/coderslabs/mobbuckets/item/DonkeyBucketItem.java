...
public class DonkeyBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_donkey";

    public DonkeyBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the donkey
            DonkeyEntity donkey = spawn(EntityType.DONKEY, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (donkey != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    donkey.readCustomDataFromTag(data);
