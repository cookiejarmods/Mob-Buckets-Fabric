...
public class PandaBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_panda";

    public PandaBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the panda
            PandaEntity panda = spawn(EntityType.PANDA, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (panda != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    panda.readCustomDataFromTag(data);
