...
public class PigBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_pig";

    public PigBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the pig
            PigEntity pig = spawn(EntityType.PIG, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (pig != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    pig.readCustomDataFromTag(data);
