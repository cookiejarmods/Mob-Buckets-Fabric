...
public class CreeperBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_creeper";

    public CreeperBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the creep
            CreeperEntity creep = spawn(EntityType.CREEPER, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (creep != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    creep.readCustomDataFromTag(data);
