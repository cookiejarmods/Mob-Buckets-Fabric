...
public class FoxBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_fox";

    public FoxBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the fox
            FoxEntity fox = spawn(EntityType.FOX, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (fox != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    fox.readCustomDataFromTag(data);
