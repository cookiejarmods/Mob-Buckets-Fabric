...
public class ParrotBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_parrot";

    public ParrotBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the parrot
            ParrotEntity parrot = spawn(EntityType.PARROT, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (parrot != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    parrot.readCustomDataFromTag(data);
