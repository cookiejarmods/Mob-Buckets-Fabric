...
public class TurtleBucketItem extends Item {

    //Thanks to charm for the code help

    public static final String STORED_MOB = "stored_turtle";

    public TurtleBucketItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient) {
            ...
            // spawn the turtle
            TurtleEntity turtle = spawn(EntityType.TURTLE, (ServerWorld)world, spawnPos, SpawnReason.BUCKET);
            if (turtle != null) {

                CompoundTag data = getCompound(held, STORED_MOB);
                if (!data.isEmpty())
                    turtle.readCustomDataFromTag(data);
